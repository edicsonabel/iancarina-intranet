      ...
      xhr.onload = function () {
        if (xhr.status === 200) {
          ...
          if (res.status == 404) {
            ...
            myDropzone_edit.options.url = res.data.imagen
            document.getElementById('titulo_edit').value = res.data.titulo
            document.getElementById('fecha_edit').value = res.data.fecha
            document.getElementById('autor_edit').value = res.data.autor
            // Establecer el contenido en el editor TinyMCE
            tinymce.get('contenido_edit').setContent(res.data.contenido)
            ...
            var imageUrl = res.data.imagen
            var extension = res.data.imagen.split('.').pop().toLowerCase()
            fetch(imageUrl)
              .then(response => response.blob())
              .then(blob => {
                var fileName = ''
                var fileName = 'imagen.' + extension
                // var fileName = res.data.imagen
